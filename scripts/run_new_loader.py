...
# settings
settings_path = 'config_disable_newton_option.yml'
...
print("RUN DEVICE", device[0])
# data
# train_gen, val_gen, test_gen, tr_steps, val_steps, test_steps, n_tr_data, n_val_data, n_test_data, normalizer, test_energy_hash = parse_ani_ccx_data(settings,'original',device[0])

parsed_data = parse_new(settings,'original',device[0]) 
...
train_batch_dataset = BatchDataset(dtrain, device[0])
...
val_batch_dataset = BatchDataset(dval, device[0])
...
test_batch_dataset = BatchDataset(dtest, device[0])
test_gen = DataLoader(test_batch_dataset, batch_size=tr_batch_size, 
                              collate_fn=collate_wrapper(env_provider=env,
                                        n_rotations=tr_rotations,
                                        freeze_rotations=settings['training']['tr_frz_rot'],
                                        keep_original=settings['training']['tr_keep_original'],
